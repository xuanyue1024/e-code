...
AI 新增或大幅修改 Java 代码时，不能只补类级或方法级 Javadoc，必须同步检查方法体内是否需要 `//` 行注释。尤其是 Service、ServiceImpl、Mapper 调用编排、导入导出、批量处理、权限校验、状态变更、缓存/Token 失效、数据脱敏、数据去重、已存在处理、级联删除、统计口径、事务边界、默认值填充、异常吞吐或跳过策略等逻辑，应在关键代码块前写简短中文注释，说明“为什么这样处理”“保护什么业务约束”“失败或冲突时采取什么策略”。查询数据、组装查询条件、转换 VO、过滤空数据、导入行级校验、导出字段脱敏、批量更新计数等常见业务代码，只要阅读者无法从单行代码立即看出业务目的，就应补注释帮助快速理解。注释密度应随业务复杂度增加；新增管理员端、后台管理、数据迁移、Excel 导入导出、权限与会话管理等代码时，默认需要比普通 CRUD 更多注释。

AI 生成代码前应先在心里明确并落实注释计划：本次新增的每个非平凡方法至少检查一次是否存在“关键分支、关键查询、关键数据处理、关键副作用”需要注释；提交前再次自查新增代码，若 ServiceImpl 中连续出现多段查询、校验、转换、写库逻辑却没有任何中文行注释，应视为未满足本规范。注释应解释意图和边界，不重复代码表面动作；例如写“导入策略是已存在跳过，避免覆盖管理员手工维护的数据”，不要写“查询用户”“设置字段”“调用 mapper”。
